    ...
    <section id="manufacturing" class="py-24 bg-slate-900 text-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-16 grid md:grid-cols-3 gap-8 text-center sm:text-left">
                ...
                <div class="bg-slate-800/40 p-6 rounded-lg border border-slate-700">
                    <div class="text-[#c5a059] mb-3">
                        <svg class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4.871 4A17.926 17.926 0 003 12c0 2.871.67 5.59 1.871 8m14.13 0a17.93 17.93 0 01-14.13 0m14.13 0A17.926 17.926 0 0021 12c0-2.871-.67-5.59-1.871-8m-14.13 0a17.93 17.93 0 0114.13 0M9 9l3 3-3 3m5 0h.01" /></svg>
                    </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid lg:grid-cols-12 gap-16">
                ...
                <div class="lg:col-span-7 bg-white p-8 rounded-xl shadow-md border border-slate-100">
                    <form id="quote-form" class="grid sm:grid-cols-2 gap-6">
                        <div>
    ...
    <script>
        window.addEventListener('DOMContentLoaded', () => {
            ...
            if(btnBaso4 && btnTalc && baso4Panel && talcPanel) {
                btnBaso4.addEventListener('click', () => {
                    baso4Panel.classList.remove('hidden');
                    talcPanel.classList.add('hidden');
                    btnBaso4.className = "px-4 py-2 text-sm rounded font-medium bg-[#c5a059] text-[#0b2c3d] transition-all";
                    btnTalc.className = "px-4 py-2 text-sm rounded font-medium text-slate-400 hover:text-white transition-all";
                });
                btnTalc.addEventListener('click', () => {
                    talcPanel.classList.remove('hidden');
                    baso4Panel.classList.add('hidden');
                    btnTalc.className = "px-4 py-2 text-sm rounded font-medium bg-[#c5a059] text-[#0b2c3d] transition-all";
            ...
            if (quoteForm) {
                quoteForm.addEventListener('submit', (e) => {
                    e.preventDefault();
                    alert('Thank you for your interest. Our B2B Sales Representative will reach out to your organization within 24 business hours.');
                    quoteForm.reset();
                });
            }
        });
...
# แปลง HTML โค้ดเป็นแบบ Base64 เพื่อตัดปัญหาข้อขัดแย้งของ Syntax Python เรื่องเครื่องหมายคำพูดคู่ (String Parsing)
b64_html = base64.b64encode(raw_html.encode('utf-8')).decode('utf-8')
html_component = f'<iframe src="data:text/html;base64,{b64_html}" width="100%" height="3700" style="border:none; overflow:hidden;" scrolling="no"></iframe>'

# แสดงผลแบบ Component Sandbox ที่ทำงานได้อย่างถูกต้อง ปลอดภัย และปุ่มกดตอบสนอง 100%
st.components.v1.html(html_component, height=3700)
